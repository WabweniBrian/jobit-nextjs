const Skeleton = () => {
  return (
    <div className="bg-slate-200 card p-3 flex-1 basis-[16rem] my-2">
      <div className="flex-align-center gap-3">
        <div className="w-14 h-14 bg-slate-300 dark:bg-hover-color animate-pulse rounded-lg" />
        <div>
          ...
          <div className="flex-align-center gap-x-4 mt-3">
            {Array.apply(null, { length: 4 }).map((_, i) => (
              <div
                className="h-4 w-[100px] bg-slate-300 dark:bg-hover-color animate-pulse rounded-md"
                key={i}
              />
            ))}
          </div>
